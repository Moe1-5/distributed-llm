...
- Status: a physical Device 2 failure is reproduced by the reported `virtual environment already exists` message. A local uv `0.9.18` dry run proves `uv venv --allow-existing` followed by `uv sync --active` preserves and syncs that existing environment. All 29 launcher/security tests and both TypeScript checks pass; a new exact package is being built for the Device 2 retry.
