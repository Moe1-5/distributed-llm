...
- Status: a temporary dry run with the installed `uv 0.9.18` proved that `uv venv` plus `uv sync --active` uses the requested external environment. All 29 launcher/security tests and both frontend TypeScript checks pass. The replacement exact package is bound to commit `b468185d87cb2884497b177e84f761478406b9af`; its strict audit reports 38 ASAR entries, 65 checksummed backend runtime files, no forbidden entries, manifest SHA-256 `a8d910a9caecac27d55a7e7ebf8ffb2e69033a869b1358f80c0ed3d535bbe447`, and acceptance identity bound. The portable executable is `87918049` bytes with SHA-256 `7385dd142b230ff226b5eddb90287f8ca29a3ba0abae52300b330915942ecb10`. Physical Device 1 launch is required before relay probing.

### 2026-08-23 - Reuse the existing managed environment during package startup

- What changed: added `--allow-existing` to the packaged `uv venv` command before `uv sync --active`.
- Why: Device 2 already contained the versioned environment directory from a prior launch, and `uv venv` refused to continue despite the next sync being able to reconcile it. Clearing it would discard reusable downloads and is unnecessary.
- Status: a physical Device 2 failure is reproduced by the reported `virtual environment already exists` message. A local uv `0.9.18` dry run proves `uv venv --allow-existing` followed by `uv sync --active` preserves and syncs that existing environment. All 29 launcher/security tests and both TypeScript checks pass. The replacement exact package is bound to commit `6b02d969ec14a143e9d3ec5e70fdc4feec3f71f8`; its strict audit reports 38 ASAR entries, 65 checksummed backend runtime files, no forbidden entries, manifest SHA-256 `cb6c5b5cf47922c639c5f4195587820cd9b4b46c889047f279896e9f490bb113`, and acceptance identity bound. The portable executable is `87919052` bytes with SHA-256 `bba876f8db2b9852b9509b84330e7de6aef07ca9134fef3e19b80f1afd9f55a9`. Device 2 must retry with this replacement.

### 2026-08-23 - Reject stale portable wrappers after cross-build

- What changed: the Windows-package audit now compares the portable executable timestamp with the packaged backend source marker and rejects a wrapper older than its unpacked runtime.
