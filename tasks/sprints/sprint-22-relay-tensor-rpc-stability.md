...
### 2026-08-23 - Confirm sustained split execution and retain terminal session-hop evidence

- What changed: recorded the current two-device adjacent-session result and extended the bounded runtime failure event with the terminal session operation, exact failed hop, peer, versioned session RPC UID, position, token count, payload-byte count, exception class, elapsed time, and already completed hops. No prompt text or tensor contents are retained.
- Why: the current run reached a healthy generator, fresh DHT discovery, healthy provider RPC, and a passing tensor canary. The response then streamed for a time before Device 1 reported `Session decode may have executed through hop 2; automatic replay is suppressed: stream reset`. The old event only retained generic route health and could not tell the next run whether the reset was on the first or second worker, or what decode position and size were affected.
- Evidence: Device 1 `0-6` and Device 2 `6-12` workers each reported 18 accepted and completed session RPCs, zero failed, rejected, or timed-out RPCs, one prefill, fourteen completed decodes, and one cancellation. This proves sustained adjacent execution before the terminal reset; it does not prove that the last dispatched decode executed, because the response was lost ambiguously. The two targeted backend unittest modules pass 30 tests, including the new exact-hop regression.
- Status: do not replay the failed prompt. Rebuild the participant artifact before the next prompt, export Settings diagnostics immediately after its single result, then run the existing stateless relay payload sweep. The exact same sweep in direct mode remains the topology control after the relayed result is preserved.
