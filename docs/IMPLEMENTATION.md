...
The client preflights the complete exact route before each operation. One strictly pre-dispatch failure may rebuild a different complete route from known token history. After an ambiguous transport reset, it may reconnect once to the same exact peer and repeat only the same operation identifier and tensor fingerprint; a bounded provider-retained result is returned without model recomputation. If that result is unavailable, the operation remains terminal with no route failover. The generator and Monitoring expose prefill/decode bytes, durations, peak cache use, retained-result recovery, and rebuild count. Stateless RPC remains versioned and available for transport comparison, unsupported architectures, and shadow/credit receipt mode; this preserves receipt idempotency until session operations gain their own signed accounting contract.

Local tests cover cached/stateless logits and greedy-token parity, constant one-position decode payloads, admission/expiry/eviction/cleanup, exact retained-result recovery after a lost response, safe rebuild, and a real exact-peer Hivemind 1.1.12 session RPC. Direct and relayed two-device session evidence remains the physical acceptance gate.
