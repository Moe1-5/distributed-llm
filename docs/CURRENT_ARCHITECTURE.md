...
`DistributedGenerator` loads local model components and performs autoregressive generation through that route. OPT providers advertise a separate peer-scoped session expert. When every hop supports protocol version one and incentives are off, the generator opens one stable exact route, sends the complete prompt once as `prefill`, sends one new position per `decode`, and explicitly closes or cancels every provider cache. A pre-dispatch route loss may rebuild once on a different complete route from the client's known token history. Any failure after a provider may have executed remains ambiguous and is never replayed automatically. The stateless expert remains the compatibility and transport-diagnostic path; shadow and credit receipt modes deliberately use it until a signed session-receipt protocol exists.

Provider cache ownership is bounded by session count, per-session bytes, total bytes, positions, operation history, and a time-to-live policy. Idle caches expire through a runtime-process cleanup loop and may be evicted least-recently-used; active caches are never evicted. Aggregate counters use multiprocessing-safe storage so the parent API and Monitoring view observe work performed inside Hivemind's forked runtime. Session, route, request, and operation identities bind every call to an exact peer, RPC UID, and layer range.
