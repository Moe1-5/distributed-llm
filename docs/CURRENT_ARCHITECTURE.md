...
`DistributedGenerator` loads local model components and performs autoregressive generation through that route. OPT providers advertise a separate peer-scoped session expert. When every hop supports protocol version one and incentives are off, the generator opens one stable exact route, sends the complete prompt once as `prefill`, sends one new position per `decode`, and explicitly closes or cancels every provider cache. A pre-dispatch route loss may rebuild once on a different complete route from the client's known token history. After an ambiguous transport reset, it may create one fresh client for the same selected peer and retry only the exact operation identifier and tensor fingerprint; the provider returns its bounded retained completion without recomputation. An unavailable retained completion remains terminal: no route failover or broad replay occurs. The stateless expert remains the compatibility and transport-diagnostic path; shadow and credit receipt modes deliberately use it until a signed session-receipt protocol exists.

Provider cache ownership is bounded by session count, per-session bytes, total bytes, positions, operation history, retained-result bytes, and a time-to-live policy. Idle caches expire through a runtime-process cleanup loop and may be evicted least-recently-used; active caches are never evicted. Aggregate counters use multiprocessing-safe storage so the parent API and Monitoring view observe work performed inside Hivemind's forked runtime. Session, route, request, operation, and tensor fingerprint identities bind every call to an exact peer, RPC UID, and layer range.

Base models preserve raw completion prompts, while chat and instruct models apply the publisher tokenizer chat template once with a generation prompt. Stream chunks are derived from cumulative tokenizer decoding so concatenating them preserves spaces and matches the final decoded sequence. The generator also supports exact generation controls, stop requests, route readiness, next-token parity probes, generated-output comparisons, and JSON trace artifacts. Generator status exposes startup/load duration, route-probe duration, first-token and per-token timing, prefill/decode wire bytes, peak provider cache use, rebuilds, token throughput, and per-hop latency aggregates.
...
OPT session protocol version one now provides distributed key/value caching for incentives-off generation. Other architectures and receipt-bearing modes retain the stateless path, and there is still no multi-generator registry. Physical direct and relayed two-device session acceptance remains open.
...
- Backend regression suite: 432 tests passing as of 2026-08-23, including adversarial RPC admission, deterministic stream-disconnect cleanup, cancellable provider-health probes, supervisor/DHT ownership races, selective layer parity, and real Hivemind normal, receipt, and session RPC integration.
- Frontend TypeScript checks, 20 managed-launcher tests, four renderer-flow tests, production build, and lint with zero errors pass. The repository still has 81 formatting warnings in pre-existing frontend files.
